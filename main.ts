export enum DIFFICULTY {
  ...
  const all_multiple_numbers: (n: number) => [number, number][] = (n) => {
  ...
  const repeat: <T extends (...args: any) => any>(n: T, times: number) => (ReturnType<T>)[] = (n, times) => {
    const arr: number[] = []
